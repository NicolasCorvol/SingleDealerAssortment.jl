...
function apply_results_to_coaml(test_benchmark_results, coaml_model)
    ...
    for res in test_benchmark_results
        ...
        results_dict_coaml = dict_solution_policies(
            instance, scenario, ["COAML"], Dict("COAML" => coaml_model)
        )
        results_other_policies["COAML"] = results_dict_coaml["COAML"]

        push!(
            all_results_with_coaml,
            OrderedDict(
                "scenario" => scenario,
                "instance" => instance,
...
function apply_multiple_res_to_coaml(
    coaml_model, log_dir_model, results_path, test_results_path
)
    for instance in results_path
        ...
        plot_boxplot_policy(
            all_results, "cost"; dir=log_dir_tes_res, name_file="boxplot_costs"
        )
        plot_boxplot_policy(
            all_results,
            "mean_replenishment";
            dir=log_dir_tes_res,
            name_file="boxplot_replenishment",
        )
        plot_mean_per_archetype(
            all_results,
            "mean_replenishment_per_archetype";
            dir=log_dir_tes_res,
            name_file="boxplot_mean_replenishment_per_archetype",
        )
        plot_gap_policies(all_results; dir=log_dir_tes_res, name_file="gap_policies")
    end
end

function compute_metrics_simulation(
    instance, scenario, replenishment, sales, y, z, elapsed_time
)
    ...
    mean_sales_over_stock_per_archetype = [
        mean([sales_over_stock[t][i] for t in 1:(instance.T)]) for i in 1:(instance.n)
    ]
    mean_replenishment_per_archetype = [
        mean([replenishment[t][i] for t in 1:(instance.T)]) for i in 1:(instance.n)
    ]
    mean_sales_per_archetype = [
        mean([sales[t][i] for t in 1:(instance.T)]) for i in 1:(instance.n)
    ]
    return (
        ...
        mean_no_buy=mean_no_buy,
        mean_sales=mean_sales,
        mean_replenishment=mean_replenishment,
        mean_replenishment_per_archetype=mean_replenishment_per_archetype,
        mean_sales_per_archetype=mean_sales_per_archetype,
        mean_dols_per_archetype=mean_dols_per_archetype,
        mean_sales_over_stock_per_archetype=mean_sales_over_stock_per_archetype,
...
function compute_gap_benchmark_policies(Y_gap::Vector{Any}, policies::Vector{String})
    ...
    thread_results = [
        Dict(policy => Dict{Int,Float64}() for policy in policies) for _ in 1:nthreads()
    ]
