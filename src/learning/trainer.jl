...
function g(y; x)
    ...
    yη = [k == 1 ? sum(z[i, j] for j in k:(x.instance.ub_same_archetype)) : - sum(
        z[i, j] for j in k:(x.instance.ub_same_archetype)
                                                                         ...
                                                                         k in
                                                                         1:(x.instance.ub_same_archetype)]
...
function train_setup(initial_model, ε, nb_samples, seed, lr; with_eta=true)
    @info "InferOpt has LinearMaximizer?" hasproperty(InferOpt, :LinearMaximizer)
    @info "Local LinearMaximizer binding?" @isdefined LinearMaximizer
    Random.seed!(seed)
...
function train_model(
    ...
    training_gaps = [compute_test_gap(Y_gap_train, model)]
    validation_gaps = [compute_test_gap(Y_gap_test, model)]
    println(
        "Initial train loss : $(train_losses[1]) \nInitial train gap : $(training_gaps[1])\nInitial validation loss : $(val_losses[1]) \nInitial test gap : $(validation_gaps[1]) \nBeginning training",
    )
    @showprogress for epoch in 1:nb_epochs
        ...
        if rem(epoch, 3) == 0
            ...
            train_gap = compute_test_gap(Y_gap_train, model)
            push!(training_gaps, train_gap)
            test_gap = compute_test_gap(Y_gap_test, model)
